  ...
  img.addEventListener("click", async () => {
    try {
      const response = await fetch(
        `https://restcountries.com/v3.1/name/${obj.name.common}`
      );
...
search_input.addEventListener("input", (event) => {
  const filtered_countries = data.filter((ctr) =>
    ctr.name.common.toLowerCase().includes(event.target.value.toLowerCase())
  );
  container.innerHTML = "";
